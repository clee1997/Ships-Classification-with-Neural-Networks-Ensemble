...
def get_augmented_loaders(df_train, df_val, input_size = (384, 384), rounds = 2):
    ...
    if rounds == 1: train_dataset = torch.utils.data.ConcatDataset([train_dataset, aug, aug_1])
    if rounds == 2: train_dataset = torch.utils.data.ConcatDataset([train_dataset, aug, aug_1, aug_2])
    if rounds == 3: train_dataset = torch.utils.data.ConcatDataset([train_dataset, aug, aug_1, aug_2, aug_3]) 
    if rounds == 4: train_dataset = torch.utils.data.ConcatDataset([train_dataset, aug, aug_1, aug_2, aug_3, aug_4]) 
